...
# Cosmo-1e Baselines
cosmo_path <- "/scratch/sadamov/wd/alnus_season_2020/mod_pollen_combined.txt"

# Measured Aggregates from the DWH for the Alnus season 2020.
load(paste0(here::here(), "/data/pollen_dwh_hourly.RData"))
load(paste0(here::here(), "/data/pollen_dwh_daily.RData"))
...
```{r read_cosmo}
data_cosmo_hourly <- read_table2(cosmo_path, col_names = TRUE) %>%
...
```{r agg_cosmo}
data_daily_cosmo <- data_cosmo_hourly %>%
...
```{r padding}
data_hourly_imp <- data_dwh_hourly %>%
  filter(between(
    datetime,
    min(data_cosmo$datetime),
    max(data_cosmo$datetime)
  )) %>%
  pad(
...
data_hourly_cosmo_imp <- data_cosmo_hourly %>%
  filter(
    between(datetime, min(data_cosmo$datetime), max(data_cosmo$datetime)),
    measurement == "concentration"
  ) %>%
  pad(
    start_val = min(data_cosmo$datetime),
...
stations_selected <- stations$station
# Check species tibble for all available pollen taxa
taxa_selected <- c("Alnus")
gg_timeseries <- list()
...
for (s in stations_selected %>% sort()) {
  ...
  gg_timeseries[[s]] <- data_timeseries %>%
    ...
    scale_colour_manual("Species", values = swatch()[c(2, 4)]) +
...
As retrieved from confluence XXXX .
