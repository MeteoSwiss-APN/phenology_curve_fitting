...
# Cosmo-1e Baselines
cosmo_path <- paste0(here::here(), "/vignettes/ext-data/mod_pollen_2020_ch.csv")
# Measured Aggregates from the DWH for the Alnus season 2020.
load(paste0(here::here(), "/data/pollen_dwh.RData"))
...
data_cosmo <- read_table2(cosmo_path, skip = 17, col_names = TRUE) %>%
  ...
  filter(between(
    datetime, as_datetime("2020-01-01 00:01:00"),
    as_datetime("2020-04-15 00:00:00")
  )) %>%
...
gg_boxplot_reldiff <- data_conc_plot %>%
  ...
  labs(
    x = "",
    y = "abs(Hirst - Cosmo) / mean(Hirst, Cosmo)",
