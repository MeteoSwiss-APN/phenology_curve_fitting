...
# library(mchdwh) # renv will not find MCHRAN, please install manually install.packages("mchdwh", repos = c("http://mchran.meteoswiss.ch", "https://stat.ethz.ch/CRAN"))
...
# This is how the dwh data in /data was retrieved. Only works on old R-3.5.2 with mchdwh library
...
There are multiple options to read in grib1/2 files directly. I prefer it this way to avoid using fxconvert or fieldextra.
Unfortunately none of the approaches found on the web worked for our laf/lfff files. I will setup a cluster running python to try other libraries.
https://stackoverflow.com/questions/37205691/reading-grib-data-in-r-reading-band-names
https://www.quora.com/How-do-I-open-GRIB-files-in-R?share=1
https://www.rdocumentation.org/packages/rNOMADS/versions/2.5.0/topics/ReadGrib
...
###   Purpose:
###
###     This converts rotated model (COSMO) coordinates into 'real' geographical coordinates (WGS84)
###   INPUT:
###
###     lat       : Latitude in degree
###     lon       : Longituede on degree
###     derotate  : logical; if TRUE lat,lon is assumed to be on COSMO grid and rotation into WGS84 will be performed
###                          if FALSE function assumes lat,lon in WGS84 and coordinates are rotated onto COSMO grid
###
###   OUTPUT:
###
###     Latitude and Longitude decimal degrees
###
###   Author: Sascha Bellaire & Reto Stauffer
#########################################################################################################
...
rotation = function(lon,lat, derotate){
  ...
  # CRS COSMO
  crs_cosmo = paste('+units=m +a=6367470 +o_proj=longlat +b=6367470',
                    '+o_lat_p=43.0 +lon_0=10.0 +proj=ob_tran +o_lon_p=0.0')

  # CRS WGS84
#  crs_wgs84 = paste('+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0')
  crs_wsg84 = paste("+proj=stere +lat_0=90 +lat_ts=90 +lon_0=10 +k=0.93301270189 +units=km +datum=WGS84")

  ##################
  ### Rotate the coordinates depending on direction
  #################

  # De-rotate from COSMO into WGS84
  if(derotate == TRUE ){

    sp_in  = SpatialPoints(as.data.frame(cbind(lon,lat)), proj4string=crs(crs_cosmo))
    sp_out = project(coordinates(sp_in)*pi/180, crs_cosmo, inv=TRUE)

  }else{ # from WGS84 into COSMO rotated grid

    sp_in  = SpatialPoints(as.data.frame(cbind(lon,lat)), proj4string=crs(crs_wgs84))
    sp_out = project(coordinates(sp_in), crs_cosmo, inv=FALSE) *180/pi
...
```{r data, echo = TRUE}
nc_path <- paste0("/scratch/sadamov/wd/osm/20021000_c1e_tsa_alnus_osm/lm_coarse/lfff00150000") # This can be any netCDF file with POAC (e.g.)
variable <- "ALNUtthrs"
...
# xy_rot <- rotation(c(x), c(y), TRUE)

# r <- raster(log10(layer),
#   xmn = min(xy_rot[, 1]), xmx = max(xy_rot[, 1]),
#   ymn = min(xy_rot[, 2]), ymx = max(xy_rot[, 2])
# )

# polygon <- layer_latlong %>%
...
plot(r, zlim = c(0, 8000), col = pal(30))
