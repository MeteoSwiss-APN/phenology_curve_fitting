...
- Wie viel zusätzliche Computing-Ressourcen werden benötigt? Wie häufig pro Tag dürfen die Pollen gerechnet werden?
...
```{r}
knitr::include_graphics(paste0(here(), "/vignettes/figures/diagram_implementation.png"))
```

Die Bedeutung der Symbole und das Diagramm stammen von Zink, 2014.

```{r out.width="75%"}
knitr::include_graphics(paste0(here(), "/vignettes/figures/parameters.png"))
```
...
- Phänologie: Beginn (und Ende) der Pollensaison von den real-time Pollendaten triggern (oder verzögern)
- Stärke der Pollensaison: Identifikation von Mastjahren und Anpassung der maximalen Emissions-Menge. 
...
- Mathematische Beschreibung der Pollen-Saison \\(f_{Q,seas}\\); Die Saisonalität der Pollenphänologie, wird im Modul pol_seasons jeweils berechnet und kann direkt optimiert werden indem die kritischen Temperatursummen laufend angepasst werden.
- Anteil der Gitterbox der mit der spezifischen Pflanze bedeckt ist \\(f_{Q,cov}\\); wird dem Modell als Grid-Inputfeld bereit gestellt (Verbreitungskarte). Momentan wird ein konstanter Tuning-Faktor in der Namelist INPUT_POL definiert. Diesen könnte man als variables Feld aufspannen. Durch einen variablen Tuningfaktor könnte dieses Feld indirekt skaliert werden. 
...
Das Modul pol_seasons wird momentan immer um 14:00 UTC aufgerufen während eines Modelllaufs, die Phänologie wird also nur einmal pro Tag aktualisiert. Das Modul pol_emissions wird in jedem Zeitschritt aufgerufen.
...
Im Modul pol_seasons wird \\(f_{Q,seas}\\) bereit gestellt. In der Subroutine *calc_saisl* des Modules könnten die zuvor geladenen Echtzeit-Daten implementiert werden. An dieser Stelle könnte das Temperatursummerfeld *tthrs_red* optimiert und variabel gemacht werden. Dieses Feld existiert bereits im Code ist aber zeitlich fixiert. Das Feld wird in data_pollen definiert, in organize_pollen alloziert und in pol_seasons mit Inhalten gefüllt. Das neue Setup ist vergleichbar mit der Art und Weise wie das Feld saisl unten berechnet wird.
...
Es wäre von Vorteil diese Routine häufiger als alle 24h auszuführen. Wenn man beispielswiese alle 3 Stunden diese Routine ausruft (also für jeden operationellen COSMO-1e Lauf), kann der Blühbeginn in Stationnähe ziemlich genau den Messungen angepasst werden. Jeweils basierend auf den letzten X-Stunden vor initial time wird für alle Stationen verglichen: Hat die Blühsaison im Modell oder in der Realität bereits begonnen. Sollten im Modell an Station X die kritischen Temperatursummen bereits erreicht worden sein, es wurden aber noch keine Pollen gemessen für diese Station, können diese Grenzwerte hochgesetzt werden (Verzögerung des Blühbeginns im Modell). In ähnlichem Vorgehen können die kritischen Summen heruntergesetzt werden, falls in Realität bereits Pollen gemessen werden, das Modell aber noch nicht mit der Emission begonnen hat.

## Die Verbreitungskarte

Wie oben erwähnt wird nicht die Karte direkt skaliert sondern viel eher könnte ein Tuning-Faktor immer bei runtime gewählt werden. Beispielsweise für Birke (siehe unten) wurde bis anhin ein konstanter Faktor *pol_tuning(1)* für das ganze Feld definiert (in INPUT_POL namelist). Anhand dieses Faktors wird no_max_day, die maximale Anzahl zu emittierbarer Pollenkörner festgelegt (im organize_pollen Modul).
...
Das Feld der Tuningfaktoren müsste initial extern bereitgestellt werden (analog zu den kritischen Temperatursummen oben). Im operationellen Modus kann es aber vom Vorlauf geerbt und allenfalls leicht angepasst werden. Bei der Anpassung sollte das Feld nicht allzu stark verändert werden, um die Kontinuität der Pollenprognosen zu gewährleisten. Dies ist erfüllt wenn immer alle verfügbare Messdaten der bisherigen Saison für die Berechnung verwendet wird.

# Interpolation

Wir werden für die Implementierung der Echtzeit Daten also zwei Gitterfelder berechnen. Zum einen werden wir das kritische Temperatursummenfeld variabel berechnen. Zum anderen wird der Tuningparameter für no_max_day auf ein Gitterfeld aufgespannt und ebenfalls variabel berechnet. Im Sinne der Transparenz, sollten beide Felder separat betrachtet werden (also keine gleichzeitige Optimierung - beispielsweise mit ML-Methoden). Während der Implementierung könnte man folgende Updates vornehmen, in zunehmender Komplexität:

- Die kritischen Temperatursummmen für den Blühbeginn werden in einem Umkreis um die Messtationen hoch- oder runter korrigiert je nachdem ob tatsächlich schon Pollen gemessen werden. Die kozentrischen Kreise können dadurch gerechtfertigt werden, dass die kritische Temperatursumme für den Blühbeginn nicht von der Topographie (MüM) abhängt. Dabei werden die Mess-Grenzwerte so definiert, dass nur lokale Emissionen und keine Transportereignisse berücksichtigt werden. Der optimale Radius der Kreise muss experimentell bestimmt werden.
...
- Falls noch mehr Zeit vorhanden sein wird, könnte man mit Kriging oder ML-Methoden die Interpolation überarbeiten. Momentan wird mit inverse distance weighting gearbeitet. Anhand von räumlicher Regression könnten Einflussgebiete der Messtationen besser verstanden und aktualisiert werden. Dabei könnten verschiedene meteorologische Wariablen wie zum Beisiel Wind berücksichtigt werden.

Für den zweiten und dritten Punkt könnten folgende Resourcen hilfreich sein:

- Die Clusteranalyse von APK um grobe Einzugsgebiete der Fallen abzuschätzen (https://www.meteoschweiz.admin.ch/content/dam/meteoswiss/de/Ungebundene-Seiten/Publikationen/Fachberichte/doc/ab237.pdf).
...
- Topographie in Voralpen wird potentiell problematisch, da nicht sehr viele Fallen zur Verfügung stehen.
- Gebiete, die zu weit weg sind von einer Messstation werden je nach Methode nicht von den Echtzeit-Daten profitieren können. Vor allem im Ausland könnte dies problematisch werden.

# Validation

Um die verschiedenen Implementierungen der variablen Felder zu vergleichen, muss eine robuste statistische Analyse durchgeführt werden. Basierend auf den Jahren 2017-2020 (COSMO-1 Pollendaten verfügbar) werden verschiedene Implementierungen miteinander verglichen. Als Ground-Truth werden die Hirst Pollenmessungen aus dem DWH verwendet. Sobald genügend Poleno Messungen verfügbar sind, sollte die Modell-Kalibrierung nachgeholt werden. Denn erste Vergleiche zeigten, dass Poleno genauere Messungen als Hirst produziert, vorallem für tiefere Konzentrationen. Zudem werden die Poleno-Geräte in einer breit anlegten Studie mit METAS, genau kalibriert. Bei Hirst muss hingegen mit varrierenden Saugraten gerechnet werden.
...
```{r}
knitr::include_graphics("figures/altman-bland.png")
```
- Density Plots

```{r}
knitr::include_graphics("figures/density_plot.png")
```

- Boxplots der relativen Differenzen

```{r}
knitr::include_graphics("figures/diff_boxplot.png")
```
...
Kategorische Metriken (Gewichtung noch nicht bekannt):
...
# Anforderungen an real-time Pollendaten
...
Da wir im Emissionsteil arbeiten werden, genügt es die Gitterboxen am nächsten am Boden zu berücksichtigen (2D-Raster). Die kostspielige 3D Simulation passiert erst im nächsten Schritt, der in diesem Projekt nicht abgeändert wird. Aus diesem Grund sollte die Implementierung der Echtzeit-Daten nicht viel Resourcen beanspruchen. Die Qualität der Prognosen darf also prioritär behandelt werden. Sollte sich allerdings zeigen, dass die Pollen häufiger als alle 24h (jeweils um 03:00 UTC in COSMO-1e) gerechnet werden müssen, um von den Echtzeitdaten zu profitieren, würde dies sehr direkt höhere Resourcen verlangen. Es wird sich in den Experimenten zeigen, ob eine häufigere Einschaltung des Pollenmoduls notwendig sein wird. COSMO-2e rechnet bereits jetzt jeden Modelllauf mit Pollen.

# Nächste Schritte
 
- Implementierung der variablen Felder Tempsummen-Felder und erste COSMO Run - wie weit zurück sollten die Daten verwendet werden und in welchem Radius um die Stationen sollte das Update vorgenommen werden?
- Finalisierung der Verifikations-Vignette und Auswahl/Gewichtung der Metriken
