...
Wie oben erwähnt wird nicht die Karte direkt skaliert sondern viel eher könnte Pol_tuning immer bei runtime definiert werden. 1.09e5_wp wurde bis anhin als Konstanter Faktor definiert für das ganze Feld.
...
Im code oberhald wird pol_tuning definiert und mit einem konstanten Wert für Birkenpollen multipliziert (anderer Wert für jede Spezies). Dadurch wird die Variable no_max_day definiert welche dann im pol_emissions Modul wieder aufgerufen wird. 
...
Im Modul pol_seasons wird \\(f_{Q,seas}\\) bereit gestellt. In der letzten Subroutine des Modules, genannt calc_sdes könnten die zuvor geladenen Echtzeit-Daten implementiert werden. An dieser Stelle könnten zwei Temperatursummerfelder optimiert werden und variabel gemacht werden (tthrs_red and tthrs).
...
# Anforderungen / Next-Steps

- Data Pipeline to retrieve the real-time pollen data, ask Dani (APN) what delay is still fine?
- Where will the validation take place? Probably best to install rstudio on tsa? 
- Use FieldExtra to analyse the model output on tsa in R?
