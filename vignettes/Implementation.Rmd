...
data_daily_imp <- data$data_daily %>%
  filter(datetime >= ymd_h("2020-01-15 00")) %>% # Start of COSMO season?
  pad(
    start_val =  ymd_h("2020-01-15 00"),
    end_val = ymd_h("2020-09-05 00"),
...
plot_comb(taxon = "Poaceae", station = "Zürich", resolution = "Daily", combined = TRUE, rm_zeros = FALSE)
...
kable_diff <- map(diff_selection,
    ...
    ungroup) %>% 
  bind_rows %>% 
  mutate(taxon = c(rep(c("Alnus", "Betula", "Poaceae"), each = 6), rep("Ambrosia", each = 2)),
