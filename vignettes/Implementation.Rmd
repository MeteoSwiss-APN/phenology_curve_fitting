...
```{r}
data_cosmo <- read_table2(paste0(here::here(), "/vignettes/ext-data/pollen.csv")) %>%
...
data_hirst_window <- data_hirst %>% 
  filter(between(datetime, as_datetime("2017-02-08 00:10:00"), as_datetime("2020-09-03 00:00:00"))) # Only look at full days
...
plot_comb(taxon = "Alnus", station = "Zürich", resolution = "Daily", combined = TRUE, rm_zeros = FALSE)
...
for (s in stations$station %>% sort){
  ...
  gg_timeseries[[s]] <- data_timeseries %>% 
    ...
    coord_cartesian(xlim = c(ymd("2017-02-08"), ymd("2020-09-03")), # Define your season here
                    ylim = c(0, 2000)) 
}
...
gg_corr <- data_corr_exp %>% 
  ...
  scale_x_continuous(name = "Measured Pollen Concentration [m⁻³]", limits = c(0, 1000)) +
  scale_y_continuous(name = "Modelled Pollen Concentration [m⁻³]", limits = c(0, 1000))
...
data_conc_plot <- data_conc %>% 
  ...
  mutate(group = factor(group, levels = c("weak", "medium", "strong", "verystrong")),
         reldiff = diff / mean) %>% 
  filter(!is.na(value))
...
gg_boxplot_reldiff <- data_conc_plot %>% 
  ...
  labs(x = "", y = "Hirst - Cosmo / mean(Hirst, Cosmo)", title = paste("Relative Pairwise Differences for Daily Average Pollen-Concentrations")) +
  facet_wrap(~ taxon)
...
kable_diff <- map(diff_selection,
    ...
    ungroup) %>% 
  bind_rows %>% 
  mutate(taxon = c(rep(c("Alnus", "Betula", "Poaceae", "Ambrosia"), each = 4)),
  ...
  pack_rows(start_row = 1, end_row = 4, indent = FALSE) %>% 
  pack_rows(start_row = 5, end_row = 8, indent = FALSE) %>% 
  pack_rows(start_row = 9, end_row = 12, indent = FALSE) %>% 
  pack_rows(start_row = 13, end_row = 16, indent = FALSE)
